  ...
  <div class="rank">
    ...
    <scroll ref="scroll" :listenScroll="true" @scroll="scrollHandler">
      <div class="official clearfix">
        ...
        <rank-row
          v-for="(item, index) in rowData.length"
          :style="{
            backgroundColor:
              index % 2 == 0
                ? index % 4 == 0
      ...
      <div
        ...
        v-for="(item, i) in titles.filter((item, index) => index != 0)"
        :key="i"
      >
        <div ref="title" :class="{ title: true, ['title' + (i + 1)]: true }">
          {{ item }}榜
        </div>
...
import { getToplist } from "../../../../network/Find/find";
...
export default {
  ...
  methods: {
        ...
        tmpTitle.forEach((it, i) => {
          ...
          tmpBlockData.every((item, index) => {
            ...
            if (i == 0) {
              ...
              //全球榜
              if (
                (index > 6 && index <= 14 && index != 9 && index != 10) ||
                index == 19
              ) {
                tmpDataArray.push(tmpData);
                if (index == 19) {
    ...
    scrollHandler: throttle(function (pos) {
      ...
      if (
        pos.y <= 0 &&
        pos.y > -this.$refs.title[0].offsetTop + baseTop &&
        this.selected != 0
      )
        this.selected = 0;
      else if (
        pos.y <= -this.$refs.title[0].offsetTop + baseTop &&
        pos.y > -this.$refs.title[1].offsetTop + baseTop &&
        this.selected != 1
      )
        this.selected = 1;
      else if (
        pos.y <= -this.$refs.title[1].offsetTop + baseTop &&
        pos.y > -this.$refs.title[2].offsetTop + baseTop &&
        this.selected != 2
      )
        this.selected = 2;
      else if (
        pos.y <= -this.$refs.title[2].offsetTop + baseTop &&
        pos.y > -this.$refs.title[3].offsetTop + baseTop &&
        this.selected != 3
      )
        this.selected = 3;
      else if (pos.y <= -this.$refs.title[3].offsetTop + baseTop)
        this.selected = 4;
    }, 60),
  },
  created() {
